  ...
  const handleSaveChanges = async () => {
    try {
      if (document.cookie.length > 0) {
        ...
        //   `http://localhost:3000/api/v1/videos/${props.id}`,formdata,
        //   {
        //     headers: {
  ...
  return (
    <div className="bg-[#272727] text-white relative  rounded-lg md:rounded-2xl p-2 m-7 w-fit">
      ...
      <div className="buttons flex flex-row md:absolute md:bottom-3  md:right-2 justify-center mb-3 md:mb-0 ">
        <button onClick={handleSaveChanges} className="bg-green-700 font-semibold tracking-tighter mx-1 rounded-lg px-5 md:px-2 py-2">
          Save <span className="hidden md:inline-block"> Changes</span>
        </button>
        <button className="bg-yellow-500  00 font-semibold tracking-tighter mx-1 rounded-lg px-5 md:px-2 py-2">
          {props.isPublished ? "Unpublish" : "Publish"}
        </button>
        <button className="bg-red-700 font-semibold tracking-tighter mx-1 rounded-lg px-5 md:px-2 py-2">
